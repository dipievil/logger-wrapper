...
func WithNotifier(n Notifier) LoggerOption {
...
func NewLoggerWrapper(config LoggerConfig, opts ...LoggerOption) *Logger {
	...
	l := &Logger{
		 slog.New(handler).With("version", config.BuildVersion, "environment", config.Environment)
	}
...
// Info logs an informational message and notifies Gotify
